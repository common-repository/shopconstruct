		...
		<tr>
			<td style="padding: 20px 40px 0 40px;">
				<div style="color:#737373;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;font-size:14px;line-height:150%;text-align:left">
					...
					<h2 style="color:#557da1;display:block;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;font-size:18px;font-weight:bold;line-height:130%;margin:16px 0 8px;text-align:left"><?php _e('Order', 'shop_ct'); echo ' #' . $data['order_data']['order']->get_id(); ?></h2>
					...
					<ul>
						...
						<li>
							...
							<span style="color:#505050;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif">
									<?php echo $data['order_data']['order']->get_billing_phone(); ?>
								</span>
						</li>
					</ul>
						...
						<tr>
							<td valign="top" width="50%">
								<h3 style="color:#557da1;display:block;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;font-size:16px;font-weight:bold;line-height:130%;margin:16px 0 8px;text-align:left">
									<?php _e('Billing details', 'shop_ct'); ?>
								</h3>
								...
								<h3 style="color:#557da1;display:block;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;font-size:16px;font-weight:bold;line-height:130%;margin:16px 0 8px;text-align:left">
									<?php _e('Shipping details', 'shop_ct'); ?>
								</h3>
								<p style="color:#505050;font-family:'Helvetica Neue',Helvetica,Roboto,Arial,sans-serif;margin:0 0 16px">
									<?php
									$shipping_first_name = $data['order_data']['order']->get_shipping_first_name();
									$shipping_last_name = $data['order_data']['order']->get_shipping_last_name();
									$shipping_company = $data['order_data']['order']->get_shipping_company();
									$shipping_address_1 = $data['order_data']['order']->get_shipping_address_1();
									$shipping_address_2 = $data['order_data']['order']->get_shipping_address_2();
									$shipping_city = $data['order_data']['order']->get_shipping_city();
									$shipping_postcode = $data['order_data']['order']->get_shipping_postcode();
									$shipping_country = $data['order_data']['order']->get_shipping_country();
									echo !empty($shipping_first_name) || !empty($shipping_last_name)
										? $shipping_first_name . ' ' . $shipping_last_name . '<br />'
										: '';
									echo !empty($shipping_company) ? $shipping_company . '<br />' : '';
									echo !empty($shipping_address_1) ? $shipping_address_1 . '<br />' : '';
									echo !empty($shipping_address_2) ? $shipping_address_2 . '<br />' : '';
									echo !empty($shipping_city) ? $shipping_city . '<br />' : '';
									echo !empty($shipping_postcode) ? $shipping_postcode . '<br />' : '';
									echo !empty($shipping_country) ? SHOP_CT()->locations->get_country_name_by_code($shipping_country) : '';
									?>
								</p>
							</td>
						</tr>
