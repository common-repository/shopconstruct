 ...
 */
final class Shop_CT_Email_Order_New extends Shop_CT_Email {
	...
	public function __construct() {
		...
		add_action( 'shop_ct_new_order', array( $this, 'send_emails' ), 10, 1 );
		add_action( 'shop_ct_email_new_order_popup', array( $this, 'init_popup' ) );
	}
	...
	public function init() {
		...
		$this->description = $this->get_option('description', 'New order emails are sent to chosen recipient(s) when a new order is received.');
		$this->subject = $this->get_option('subject', __('New Order','shop_ct'));
		$this->heading = $this->get_option('heading', __('New Order','shop_ct'));
		$this->message = $this->get_option('message', __('You have received a new order from a customer. Here are the order details: ','shop_ct'));
		$this->customer_subject = $this->get_option('customer_subject', __('Order Received','shop_ct'));
		$this->customer_heading = $this->get_option('customer_heading', __('Order Received','shop_ct'));
		$this->customer_message = $this->get_option('customer_message', __('Hi there. Your order has been received and is now being processed. See your order details below for your reference:','shop_ct'));
	}
	...
	public function send_email( $for_admin, $data, $receiver ) {
		...
		if ( $for_admin ) {
			$result = array(
				'email' => wp_mail( $receiver, $this->subject, $html, $headers ),
				'html'  => $html,
			);
		} else {
