 ...
 */
final class Shop_CT_Email_Order_Failed extends Shop_CT_Email {
	 ...
	 * @var string
	 */
	public static $action = 'shop_ct_failed_order';
	...
	public function __construct() {
		...
		add_action( 'shop_ct_failed_order', array( $this, 'send_emails' ), 10, 1 );
		add_action( 'shop_ct_email_failed_order_popup', array( $this, 'init_popup' ) );
	}
	...
	public function init() {
		...
		$this->description = $this->get_option('description', 'An email is sent to chosen recipient(s) when the order failed to proceed');
		$this->subject = $this->get_option('subject', __('Order failed','shop_ct'));
		$this->heading = $this->get_option('heading', __('Order failed','shop_ct'));
		$this->message = $this->get_option('message', __('Order failed','shop_ct'));
		$this->customer_subject = $this->get_option('customer_subject', __('Order failed','shop_ct'));
		$this->customer_heading = $this->get_option('customer_heading', __('Order failed','shop_ct'));
		$this->customer_message = $this->get_option('customer_message', __('Order failed','shop_ct'));
	}
