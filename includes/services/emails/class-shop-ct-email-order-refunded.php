 ...
 */
final class Shop_CT_Email_Order_Refunded extends Shop_CT_Email {
	 ...
	 * @var string
	 */
	public static $action = 'shop_ct_refunded_order';
	...
	public function __construct() {
		...
		add_action( 'shop_ct_refunded_order', array( $this, 'send_emails' ), 10, 1 );
		add_action( 'shop_ct_email_refunded_order_popup', array( $this, 'init_popup' ) );
	}
	...
	public function init() {
		...
		$this->description = $this->get_option('description', 'An email is sent to chosen recipients when the order marked as refunded');
		$this->subject = $this->get_option('subject', __('Order Refunded','shop_ct'));
		$this->heading = $this->get_option('heading', __('Order Refunded','shop_ct'));
		$this->message = $this->get_option('message', __('Order Refunded','shop_ct'));
		$this->customer_subject = $this->get_option('customer_subject', __('Order Refunded','shop_ct'));
		$this->customer_heading = $this->get_option('customer_heading', __('Order Refunded','shop_ct'));
		$this->customer_message = $this->get_option('customer_message', __('Order Refunded','shop_ct'));
	}
	...
	public function send_emails($data) {
		...
		foreach ($receivers as $receiver) {
			...
			$this->send_email(false !== strpos($receiver, "{customer}"), $data, $receiver);
		}
	}
	...
	public function send_email($for_admin, $data, $receiver) {
		...
		$html = $this->buildHtml($template_path, 'common', array(
			'message' => $message,
			'heading' => $heading,
			'data'    => $data,
		));
