...
class Geolocation
{
    ...
    public static function init() {
        ...
        if ( 'geolocation' === get_option( 'shop_ct_settings_base_location' ) || apply_filters( 'shop_ct_geolocation_update_database_periodically', false ) ) {
            add_action( 'shop_ct_geoip_updater', array( __CLASS__, 'update_database' ) );
        }
        add_filter( 'pre_update_option_shop_ct_default_customer_address', array( __CLASS__, 'maybe_update_database' ), 10, 2 );
    }
    ...
    public static function get_external_ip_address() {
        ...
        if ( false === $external_ip_address ) {
            ...
            $ip_lookup_services      = apply_filters( 'shop_ct_geolocation_ip_lookup_apis', self::$ip_lookup_apis );
            ...
            foreach ( $ip_lookup_services_keys as $service_name ) {
                ...
                if ( ! is_wp_error( $response ) && $response['body'] ) {
                    $external_ip_address = apply_filters( 'shop_ct_geolocation_ip_lookup_api_response', Shop_CT_Formatting::clean( $response['body'] ), $service_name );
                    break;
                }
            }
    ...
    private static function geolocate_via_api( $ip_address ) {
        ...
        if ( false === $country_code ) {
            $geoip_services      = apply_filters( 'shop_ct_geolocation_geoip_apis', self::$geoip_apis );
                ...
                if ( ! is_wp_error( $response ) && $response['body'] ) {
                    switch ( $service_name ) {
                        ...
                        case 'telize' :
                            ...
                            break;
                        default :
                            $country_code = apply_filters( 'shop_ct_geolocation_geoip_response_' . $service_name, '', $response['body'] );
                            break;
                    }
